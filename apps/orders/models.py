...
class OrderItem(models.Model):
    product = models.ForeignKey(ProductUnidad, on_delete=models.DO_NOTHING)
    order = models.ForeignKey(Order, on_delete=models.CASCADE)
    name = models.CharField(max_length=255)
    price = models.DecimalField(max_digits=5, decimal_places=2)
    count = models.IntegerField()
    date_added = models.DateTimeField(default=datetime.now)
    talla = models.CharField(max_length=50, null=True, blank=True)  # Talla del producto
    color = models.CharField(max_length=50, null=True, blank=True)  # Color del producto
